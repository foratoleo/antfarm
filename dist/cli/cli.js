...
async function main() {
    ...
    if (action === "resume") {
        ...
        const run = db.prepare("SELECT id, workflow_id, status FROM runs WHERE id = ? OR id LIKE ?").get(target, `${target}%`);
        if (!run) {
            process.stderr.write(`Run not found: ${target}\n`);
            process.exit(1);
        }
        ...
        const failedStep = db.prepare("SELECT id, step_id, type, current_story_id FROM steps WHERE run_id = ? AND status = 'failed' ORDER BY step_index ASC LIMIT 1").get(run.id);
        if (!failedStep) {
        ...
        if (failedStep.type === "loop") {
            const failedStory = db.prepare("SELECT id FROM stories WHERE run_id = ? AND status = 'failed' ORDER BY story_index ASC LIMIT 1").get(run.id);
            if (failedStory) {
                db.prepare("UPDATE stories SET status = 'pending', updated_at = datetime('now') WHERE id = ?").run(failedStory.id);
            }
        }
        // Check if the failed step is a verify step linked to a loop step's verify_each
        const loopStep = db.prepare("SELECT id, loop_config FROM steps WHERE run_id = ? AND type = 'loop' AND status IN ('running', 'failed') LIMIT 1").get(run.id);
        if (loopStep?.loop_config) {
            ...
            if (lc.verifyEach && lc.verifyStep === failedStep.step_id) {
                ...
                db.prepare("UPDATE steps SET status = 'pending', current_story_id = NULL, updated_at = datetime('now') WHERE id = ?").run(loopStep.id);
                // Reset verify step to waiting (fires after developer completes)
                db.prepare("UPDATE steps SET status = 'waiting', current_story_id = NULL, updated_at = datetime('now') WHERE id = ?").run(failedStep.id);
                // Reset any failed stories to pending
                db.prepare("UPDATE stories SET status = 'pending', updated_at = datetime('now') WHERE run_id = ? AND status = 'failed'").run(run.id);
        ...
        db.prepare("UPDATE steps SET status = 'pending', current_story_id = NULL, updated_at = datetime('now') WHERE id = ?").run(failedStep.id);
        // Reset run to running
        db.prepare("UPDATE runs SET status = 'running', updated_at = datetime('now') WHERE id = ?").run(run.id);
        // Ensure crons are running for this workflow
        const { loadWorkflowSpec } = await import("../installer/workflow-spec.js");
    ...
    if (action === "run") {
        ...
        process.stdout.write([`Run: ${run.id}`, `Workflow: ${run.workflowId}`, `Task: ${run.task}`, `Status: ${run.status}`].join("\n") + "\n");
        return;
    }
