...
function getRuns(workflowId) {
    ...
    const runs = workflowId
        ? db.prepare("SELECT * FROM runs WHERE workflow_id = ? ORDER BY created_at DESC").all(workflowId)
        : db.prepare("SELECT * FROM runs ORDER BY created_at DESC").all();
    return runs.map((r) => {
        const steps = db.prepare("SELECT * FROM steps WHERE run_id = ? ORDER BY step_index ASC").all(r.id);
        return { ...r, steps };
    });
}
