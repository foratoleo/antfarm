...
const SUBAGENT_POLICY = { allowAgents: [] };
/**
...
function inferRole(agentId) {
    ...
    if (id.includes("planner") || id.includes("prioritizer") || id.includes("reviewer")
        || id.includes("investigator") || id.includes("triager"))
        return "analysis";
    if (id.includes("verifier"))
        return "verification";
    if (id.includes("tester"))
        return "testing";
    if (id.includes("scanner"))
        return "scanning";
    if (id === "pr" || id.includes("/pr"))
        return "pr";
    // developer, fixer, setup → coding
    return "coding";
}
function buildToolsConfig(role) {
    const policy = ROLE_TOOL_POLICIES[role];
    const tools = {};
    if (policy.profile)
        tools.profile = policy.profile;
    if (policy.alsoAllow?.length)
        tools.alsoAllow = policy.alsoAllow;
