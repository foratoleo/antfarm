...
const UPDATE_HINT = `This may be fixed by updating OpenClaw: npm update -g openclaw`;
// ---------------------------------------------------------------------------
...
async function createAgentCronJobHTTP(job) {
    ...
    try {
        ...
        if (response.status === 404)
            return null; // signal CLI fallback
        if (!response.ok) {
 ...
 */
export async function checkCronToolAvailable() {
    // Try HTTP
    const gateway = await getGatewayConfig();
    try {
        const headers = { "Content-Type": "application/json" };
...
async function listCronJobsHTTP() {
    const gateway = await getGatewayConfig();
    try {
        const headers = { "Content-Type": "application/json" };
        if (gateway.token)
...
async function deleteCronJobHTTP(jobId) {
    const gateway = await getGatewayConfig();
    try {
        const headers = { "Content-Type": "application/json" };
        if (gateway.token)
...
export async function deleteAgentCronJobs(namePrefix) {
