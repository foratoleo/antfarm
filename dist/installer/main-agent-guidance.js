...
function resolveMainAgentWorkspacePath(cfg) {
    const workspace = cfg.agents?.defaults?.workspace?.trim();
    if (workspace)
        return workspace;
    return path.join(process.env.HOME ?? "", ".openclaw", "workspace");
}
export async function updateMainAgentGuidance() {
    const { config } = await readOpenClawConfig();
    const workspaceDir = resolveMainAgentWorkspacePath(config);
...
export async function removeMainAgentGuidance() {
    const { config } = await readOpenClawConfig();
    const workspaceDir = resolveMainAgentWorkspacePath(config);
    const toolsPath = path.join(workspaceDir, "TOOLS.md");
    const agentsPath = path.join(workspaceDir, "AGENTS.md");
    ...
    const agentsContent = await readFileOrEmpty(agentsPath);
    if (toolsContent)
        await fs.writeFile(toolsPath, removeBlock(toolsContent), "utf-8");
    if (agentsContent)
        await fs.writeFile(agentsPath, removeBlock(agentsContent), "utf-8");
}
