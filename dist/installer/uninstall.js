...
import { resolveAntfarmRoot, resolveWorkflowDir, resolveWorkflowWorkspaceDir, resolveWorkflowWorkspaceRoot, resolveWorkflowRoot, } from "./paths.js";
...
function filterAgentList(list, workflowId) {
...
function getActiveRuns(workflowId) {
    try {
        const db = getDb();
        if (workflowId) {
            return db.prepare("SELECT id, workflow_id, task FROM runs WHERE workflow_id = ? AND status = 'running'").all(workflowId);
        }
        return db.prepare("SELECT id, workflow_id, task FROM runs WHERE status = 'running'").all();
    }
    catch {
        return [];
    }
}
export function checkActiveRuns(workflowId) {
    return getActiveRuns(workflowId);
}
function removeRunRecords(workflowId) {
    try {
        const db = getDb();
        const runs = db.prepare("SELECT id FROM runs WHERE workflow_id = ?").all(workflowId);
...
export async function uninstallWorkflow(params) {
    ...
    removeSubagentAllowlist(config, removedAgents
...
export async function uninstallAllWorkflows() {
    const { path: configPath, config } = await readOpenClawConfig();
    const list = Array.isArray(config.agents?.list) ? config.agents?.list : [];
    ...
    if (await pathExists(antfarmRoot)) {
        const entries = await fs.readdir(antfarmRoot).catch(() => []);
        if (entries.length === 0) {
