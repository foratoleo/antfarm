 ...
 */
function cleanupAbandonedSteps() {
    const db = getDb();
    const cutoff = new Date(Date.now() - ABANDONED_THRESHOLD_MS).toISOString();
    // Find running steps that haven't been updated recently
    const abandonedSteps = db.prepare("SELECT id, step_id, run_id, retry_count, max_retries FROM steps WHERE status = 'running' AND updated_at < ?").all(cutoff);
    for (const step of abandonedSteps) {
        const newRetry = step.retry_count + 1;
        if (newRetry >= step.max_retries) {
            // Fail the step and run
            db.prepare("UPDATE steps SET status = 'failed', output = 'Agent abandoned step without completing', retry_count = ?, updated_at = datetime('now') WHERE id = ?").run(newRetry, step.id);
    ...
    const abandonedStories = db.prepare("SELECT id, retry_count, max_retries, run_id FROM stories WHERE status = 'running' AND updated_at < ?").all(cutoff);
...
export function claimStep(agentId) {
    ...
    const db = getDb();
    const step = db.prepare("SELECT id, run_id, input_template, type, loop_config FROM steps WHERE agent_id = ? AND status = 'pending' LIMIT 1").get(agentId);
    if (!step)
        return { found: false };
    // Get run context
...
export function completeStep(stepId, output) {
    ...
    const loopStepRow = db.prepare("SELECT id, loop_config, run_id FROM steps WHERE run_id = ? AND type = 'loop' AND status = 'running' LIMIT 1").get(step.run_id);
 ...
 */
export function failStep(stepId, error) {
    const db = getDb();
    const step = db.prepare("SELECT run_id, retry_count, max_retries, type, current_story_id FROM steps WHERE id = ?").get(stepId);
    if (!step)
        throw new Error(`Step not found: ${stepId}`);
